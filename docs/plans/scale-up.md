...
Cogniva hiện là Next.js monolith ổn trên Vercel + Postgres + Socket.IO self-host (gateway `apps/realtime`). Stack này handle được **~500 concurrent / 5K MAU** trước khi bắt đầu vỡ. Để scale lên **100K-1M concurrent** trong 18 tháng, hệ thống phải tiến hoá qua **3 stage** thay vì rewrite-from-scratch:
...
| Inngest                    | $0/free → $250/mo       | $250 cần khi > 50K function runs | So sánh lịch sử — hiện đã dùng BullMQ (Redis self-host, $0) thay Inngest |
...
- BullMQ (Redis self-host) cho background job
...
const ROUTES: Record<UseCase, Route> = {
  ragChat: {
    ...
    budget: { maxCostUsd: 0.1, maxLatencyMs: 30_000 },
    cache: { ttl: 300, semantic: true },
    evalGate: { faithfulness: 0.85, relevancy: 0.8 },
  },
  // ... more routes
};
```
...
**Live exam (Phase 17 from rooms-and-exam.md, scale considerations):**
...
async function shadowCompare<T>(
  ...
  const [primaryResult, shadowResult] = await Promise.allSettled([primary(), shadow()]);
...
| T2  | Socket.IO gateway (apps/realtime) single-node failure → realtime down | 3       | 5       | 15    | Redis adapter, scale N replica + monitor health | DevOps  | M2      |
...
_Cogniva Scale-Up Master Plan **v2.0** — 18-month roadmap from MVP to 600K MAU big-tech-grade platform._  
_Last updated: 2026-05-11_  
_Authors: Cogniva team + AI architect review_  
_Status: Living document. Update each retro + ADR addition._
