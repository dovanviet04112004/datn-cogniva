/**
 * Concierge Agent — V4 T1 (2026-05-22).
 *
 * AI Concierge cho marketplace gia sư. Flow 2 bước:
 *
 *   1. PLANNER (Haiku, ~50 tok): phân tích last user message + history →
 *      quyết định ACTION:
 *        - 'clarify'  : hỏi thêm thông tin (chưa đủ subject/level/budget)
 *        - 'search'   : đủ thông tin, gọi searchTutors
 *        - 'no_match' : đã search nhưng không khớp gì, đề xuất đăng request
 *
 *   2. RESPONDER (Haiku, streaming): viết câu trả lời tiếng Việt thân thiện
 *      kèm match reason cho từng tutor.
 *
 * Lý do tách 2 bước:
 *   - Planner deterministic JSON → backend parse, gọi search engine
 *   - Responder stream tự nhiên cho UX nhanh
 *   - Mỗi turn chỉ tốn ~200 token Haiku (≈ $0.001) → cheap
 *
...
import { inferSubjectFromText, inferLevelFromText } from './subject-infer';
 ...
 */
export async function planConciergeStep({
  history,
  currentFilters,
  userId,
  plan,
  ...
  // Parse — strip markdown fences nếu Haiku thêm vào
  const cleaned = text
    .trim()
    .replace(/^```(?:json)?\s*/, '')
    .replace(/\s*```$/, '');
  ...
  try {
    ...
    // Lý do: AI router dùng Groq llama-3.1-8b / gpt-oss-20b / gemini-flash —
    // các model nhỏ không follow rule "có subject thì search" 100%. Khi planner
    // trả clarify dù user message hoặc filter cũ ĐÃ có subject → override
    // sang action=search. Áp dụng cho cả student + tutor side.
 ...
 * Batch 5 tutor / call. Stream để UI render dần.
 */
 ...
 * Trả null nếu invalid (planner hallucinate slug không có).
 */
