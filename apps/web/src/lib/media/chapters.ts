/**
 * Auto-chapter detection — chia transcript thành các chương theo topic shift.
 *
 * Thuật toán (heuristic v1):
 *   1. Group segments thành "blocks" 60-90s liên tiếp (≈ 1 đoạn nói tự nhiên).
 *   2. Mỗi block embed bằng Voyage AI (cùng provider chunks pipeline).
 *   3. Cosine similarity giữa block N và N-1 — nếu < threshold (0.65) →
 *      đánh dấu là chapter boundary.
 *   4. Ghép boundary → chương: title = AI-gen ngắn từ 3 câu đầu của chương.
 *
 * Vì sao không dùng LLM end-to-end gen chapter:
 *   - LLM trên transcript 60 phút (~10K từ) → context $1+ cost mỗi lần.
 *   - Embedding-based: $0.01/lần, nhanh, deterministic.
 *
 * V2 (Phase 18+) sẽ thêm LLM gen title quality cao hơn — hiện v1 dùng
 * sentence-start làm title để pipeline chạy được standalone không cần extra
 * LLM call.
 *
 * Output dùng cho Replay UI: chapter list bên phải, click → seek video.
 */
import type { TranscribeSegment } from './whisper';
...
/**
 * Cosine similarity giữa 2 vector cùng số chiều.
 * Trả -1 nếu zero vector (tránh chia 0).
 */
 ...
 * Mỗi block có start (= segment đầu), end (= segment cuối), text concat.
 */
...
/**
 * Tạo title từ block đầu của chapter — lấy 6-10 từ đầu, không có thì rỗng.
 * Format: capitalize chữ cái đầu, bỏ dấu câu cuối.
 */
