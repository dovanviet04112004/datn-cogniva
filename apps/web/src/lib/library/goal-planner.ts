/**
 * goal-planner — V1 Pillar #1 (2026-05-22).
 *
 * Parse mục tiêu user → StudyGoal struct → build kế hoạch study + library docs.
 *
 * Flow:
 *   1. parseGoal(userMessage) — LLM extract subject/level/grade/deadline/score
 *   2. buildStudyPlan(goal) — chia thời gian thành tuần, mỗi tuần focus 1 cluster
 *   3. searchLibraryDocsForPlan(plan) — search docs cho từng cluster
 *
 * Spec: docs/plans/library-share.md §Pillar #1 Goal-Driven Discovery.
 */
import { z } from 'zod';

import { routedGenerateText } from '@/lib/ai/router';

import { hybridSearchLibraryDocs, type LibraryDocResult } from './hybrid-search-doc';
...
export async function parseGoal({
  ...
  try {
    const { text } = await routedGenerateText({
      ...
      messages: [{ role: 'user', content: `Mục tiêu user:\n${userMessage}\n\nTrả JSON.` }],
      maxOutputTokens: 300,
...
// ─── Topic clusters cho mỗi subject ──────────────────────────────────
/**
 * Mỗi subject có blueprint các "cluster" để phân chia plan theo tuần.
 * Curated theo syllabus VN, có thể override qua focusTopics của user.
 */
 ...
 *
 * Strategy:
 *   - Chọn N cluster phù hợp goalType (graduation = full, gifted = nâng cao, ...)
 *   - Chia đều thời gian: N cluster / deadlineWeeks
 *   - Ưu tiên focusTopics nếu user mention
 *   - Search library cho mỗi cluster (3 categories: theory, exercise, exam)
 */
export async function buildStudyPlan(goal: StudyGoal): Promise<StudyPlanResult> {
  ...
  // Reserve tuần cuối cho đề thi nếu plan ≥ 3 tuần và là exam-style goal
