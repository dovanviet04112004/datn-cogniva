/**
 * Inngest function `flashcard-due-reminder` — daily push notification.
 *
...
export const flashcardDueReminder = inngest.createFunction(
  {
    id: 'flashcard-due-reminder',
    name: 'Daily flashcard due reminder push',
    retries: 1, // Cron retry once nếu fail, không retry vô hạn vì sẽ spam
  },
  // 13:00 UTC daily = 20:00 VN (UTC+7). Cron syntax: phút giờ ngày tháng dow.
  { cron: '0 13 * * *' },
  async ({ step, logger: inngestLogger }) => {
    ...
    const candidates = await step.run('query-candidates', async () => {
      const rows = await db
        .select({
    ...
    inngestLogger.info(`Found ${candidates.length} users with due cards ≥ ${MIN_DUE_THRESHOLD}`);
    ...
    const dedupedUserIds = await step.run('dedupe-recent-sent', async () => {
    ...
    inngestLogger.info(`After dedupe: ${dedupedUserIds.length} users eligible`);
    ...
    const targets = await step.run('lookup-tokens', async () => {
    ...
    if (targets.length === 0) {
      inngestLogger.warn('No push tokens found for eligible users');
    ...
    // KHÔNG dùng step.run cho từng batch vì Inngest log mỗi step → spam.
    // 1 step cho toàn bộ HTTP work, log result tổng.
    ...
    if (sendResult.invalidTokens.length > 0) {
      await step.run('cleanup-invalid-tokens', async () => {
        await db
          .delete(pushToken)
    ...
    await step.run('insert-notification-logs', async () => {
