...
export function RoomShareCode({ roomId, joinCode }: Props) {
  ...
  const copy = async (kind: 'code' | 'link') => {
    try {
      const text = kind === 'code' ? joinCode : `${window.location.origin}/rooms/${roomId}/lobby`;
  ...
  return (
    <Card>
      <CardContent className="flex flex-wrap items-center gap-3 py-3">
        ...
        <button
          onClick={() => copy('code')}
          className="bg-muted hover:bg-muted/70 flex items-center gap-1.5 rounded-md border px-3 py-1 font-mono text-sm tracking-wider transition-colors"
          aria-label="Copy mã"
        >
          {joinCode}
          {copied === 'code' ? (
