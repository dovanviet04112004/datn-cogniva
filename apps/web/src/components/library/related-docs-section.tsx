...
export function RelatedDocsSection({
  ...
  const { data: related = [], isLoading: loading } = useQuery({
    ...
    queryFn: () =>
      apiGet<{ related: RelatedDoc[] }>(`/api/library/docs/${sourceDocId}/related`).then(
        (d) => d.related,
      ),
  });
    ...
    try {
      ...
      toast.success(
        `${t('library.related.imported').replace('{imported}', String(data.imported))}${
          data.skipped
            ? t('library.related.skipped').replace('{skipped}', String(data.skipped))
            : ''
  ...
  if (loading) {
    return (
      <section className="mt-8 space-y-3">
        ...
        <div className="border-divider bg-card text-muted-foreground rounded-xl border p-4 text-[12.5px]">
    ...
    <section className="mt-8 space-y-3">
      <SectionHeading
        ...
        action={
          <button
            type="button"
            onClick={toggleAll}
            className="text-discovery-600 hover:text-discovery-700 dark:text-discovery-300 text-[11px] font-medium"
          >
      ...
      >
        <span className="inline-flex items-center gap-2">
          <Sparkles className="text-discovery-600 h-3.5 w-3.5" />
          {t('library.related.section_title')}
        </span>
      </SectionHeading>
      <p className="text-muted-foreground text-[12px]">{t('library.related.subtitle')}</p>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <input
                    ...
                    className="accent-discovery-600 mt-0.5 h-3.5 w-3.5 shrink-0"
                  />
                </div>
                <p className="line-clamp-2 text-[12.5px] font-semibold leading-snug">{r.title}</p>
                {r.atomOverlap > 0 ? (
                  <p className="text-discovery-700 dark:text-discovery-300 text-[10.5px] font-medium">
                ...
                <div className="text-muted-foreground mt-auto flex items-center justify-between text-[10.5px]">
                  <span>
                    {r.pageCount ?? '–'} {t('library.related.page_unit')}
                  </span>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t('library.related.choose_ws_title').replace(
                '{count}',
                String(selectedIds.size + 1),
              )}
            </DialogTitle>
          </DialogHeader>
          ...
          {wsLoading ? (
            ...
            <div className="py-2">
              <p className="text-muted-foreground text-[13px]">{t('library.related.no_ws')}</p>
            </div>
          ) : (
