/**
 * ExamPreviewContext — V8.21 (2026-05-20).
 *
 * State mở `<ExamEditorDialog>` cho 1 exam id cụ thể. Cộng thêm version
 * counter để Sources panel refetch exam list sau khi publish/delete.
 *
 * Pattern giống NotePreviewContext (notesVersion bump).
 */
'use client';
...
export type ExamPreviewContextValue = {
  ...
  /** Bump khi exam thay đổi (publish/delete) → Studio refetch list. */
  examsVersion: number;
...
const ExamPreviewContext = React.createContext<ExamPreviewContextValue | null>(
  null,
);
...
export function ExamPreviewProvider({
  children,
}: {
  ...
  const open = React.useCallback((id: string) => {
    setExamId(id);
    setModeState('inline'); // default mở vào Studio inline preview
  }, []);
  const setMode = React.useCallback(
    (m: ExamPreviewMode) => setModeState(m),
    [],
  );
  ...
  const bumpExamsVersion = React.useCallback(
  ...
  return (
    <ExamPreviewContext.Provider value={value}>
      {children}
