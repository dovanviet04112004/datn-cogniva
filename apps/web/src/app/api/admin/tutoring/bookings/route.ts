/**
 * GET /api/admin/tutoring/bookings — list bookings cross-marketplace với filter.
 *
 * Query params:
 *   q          — substring search trên student/tutor email
 *   status     — PENDING_TUTOR | CONFIRMED | IN_PROGRESS | COMPLETED | CANCELLED
 *   cursor     — startAt ISO row cuối trang trước (newest first)
 *   limit      — default 50, max 100
 *
 * Trả kèm payment.status nếu có để admin nhanh chóng đánh giá tình huống.
 */
import { NextResponse } from 'next/server';
import { aliasedTable, and, desc, eq, ilike, lt, or, sql } from 'drizzle-orm';

import {
  db,
  tutorProfile,
  tutoringBooking,
  tutoringPayment,
  user,
} from '@cogniva/db';

import { isGuardResponse, requireAdminRole } from '@/lib/admin/guard';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const MAX_LIMIT = 100;

// Alias user cho tutor (join qua tutorProfile.userId)
const tutorUser = aliasedTable(user, 'tutor_u');
const studentUser = aliasedTable(user, 'student_u');

export async function GET(request: Request) {
  try {
    await requireAdminRole();
  } catch (err) {
    if (isGuardResponse(err)) return err;
    throw err;
  }

  const url = new URL(request.url);
  const q = url.searchParams.get('q')?.trim() ?? '';
  const status = url.searchParams.get('status');
  const cursor = url.searchParams.get('cursor');
  const limitRaw = Number(url.searchParams.get('limit') ?? 50);
  const limit = Number.isFinite(limitRaw)
    ? Math.min(MAX_LIMIT, Math.max(1, Math.floor(limitRaw)))
    : 50;

  const conditions = [] as Parameters<typeof and>[number][];
  if (q) {
    conditions.push(
      or(ilike(tutorUser.email, `%${q}%`), ilike(studentUser.email, `%${q}%`))!,
    );
  }
  if (status) conditions.push(eq(tutoringBooking.status, status));
  if (cursor) {
    const parsed = new Date(cursor);
    if (!Number.isNaN(parsed.getTime())) {
      conditions.push(lt(tutoringBooking.startAt, parsed));
    }
  }

  const rows = await db
    .select({
      id: tutoringBooking.id,
      status: tutoringBooking.status,
      subjectSlug: tutoringBooking.subjectSlug,
      level: tutoringBooking.level,
      startAt: tutoringBooking.startAt,
      endAt: tutoringBooking.endAt,
      rateVnd: tutoringBooking.rateVnd,
      createdAt: tutoringBooking.createdAt,
      cancelledAt: tutoringBooking.cancelledAt,
      cancelledBy: tutoringBooking.cancelledBy,
      tutorProfileId: tutoringBooking.tutorId,
      tutorUserId: tutorProfile.userId,
      tutorName: tutorUser.name,
      tutorEmail: tutorUser.email,
      studentId: tutoringBooking.studentId,
      studentName: studentUser.name,
      studentEmail: studentUser.email,
      paymentStatus: tutoringPayment.status,
      paymentProvider: tutoringPayment.provider,
      paymentAmountVnd: tutoringPayment.amountVnd,
    })
    .from(tutoringBooking)
    .leftJoin(tutorProfile, eq(tutorProfile.id, tutoringBooking.tutorId))
    .leftJoin(tutorUser, eq(tutorUser.id, tutorProfile.userId))
    .leftJoin(studentUser, eq(studentUser.id, tutoringBooking.studentId))
    .leftJoin(tutoringPayment, eq(tutoringPayment.bookingId, tutoringBooking.id))
    .where(conditions.length > 0 ? and(...conditions) : undefined)
    .orderBy(desc(tutoringBooking.startAt))
    .limit(limit + 1);

  const hasMore = rows.length > limit;
  const trimmed = hasMore ? rows.slice(0, limit) : rows;
  const nextCursor =
    hasMore && trimmed.length > 0
      ? trimmed[trimmed.length - 1]!.startAt.toISOString()
      : null;

  let total: number | null = null;
  if (conditions.length === 0) {
    const [r] = await db.execute<{ n: number }>(
      sql`SELECT COUNT(*)::int AS n FROM "tutoring_booking"`,
    );
    total = Number(r?.n ?? 0);
  }

  return NextResponse.json({
    bookings: trimmed.map((b) => ({
      ...b,
      startAt: b.startAt.toISOString(),
      endAt: b.endAt.toISOString(),
      createdAt: b.createdAt.toISOString(),
      cancelledAt: b.cancelledAt?.toISOString() ?? null,
    })),
    nextCursor,
    total,
  });
}
