/**
 * /library/university/[id] — University landing kiểu Studocu (2026-05-28).
 *
 * Header trường + browse môn (search + tab A-Z/Phổ biến + grid folder) + sidebar
 * "Danh mục nội dung" (tổng doc + breakdown theo loại). Click môn → trang môn.
 */
  ...
  return (
    <PageShell size="wide">
      ...
      <div className="mb-3 flex items-center gap-1.5 text-[12px] text-muted-foreground">
        ...
        <Link href="/library/universities" className="inline-flex items-center gap-1 hover:text-foreground">
          <Compass className="h-3 w-3" />
          {t('library.browse.title')}
        </Link>
        <span className="opacity-40">/</span>
        <span className="truncate font-medium text-foreground/80">{uni.shortName ?? uni.name}</span>
      </div>

      {/* University header */}
      <header className="mb-6 flex flex-col gap-3 rounded-2xl border border-divider bg-gradient-to-br from-sky-500/8 via-card to-card p-5 lg:flex-row lg:items-center lg:justify-between">
      ...
      {/* Main 2-col: courses browser + content categories sidebar */}
      <div className="grid gap-6 lg:grid-cols-[1fr_260px]">
        <div className="min-w-0">
          ...
          {courses.length > 0 ? (
            ...
            <p className="rounded-xl border border-dashed border-divider py-10 text-center text-[13px] text-muted-foreground">
          ...
          <div className="rounded-2xl border border-divider bg-card p-4">
            ...
            <dl className="space-y-2.5">
              <div className="flex items-baseline justify-between border-b border-divider pb-2.5">
                ...
                <dd className="font-mono text-[15px] font-bold tabular-nums text-discovery-600">
                  {uni.docCount}
                </dd>
              </div>
              {docTypeBreakdown.map((r) => (
                <div key={r.docType} className="flex items-baseline justify-between">
                  ...
                  <dd className="font-mono text-[12.5px] tabular-nums text-foreground/80">
                    {Number(r.n)}
                  </dd>
                </div>
              ))}
