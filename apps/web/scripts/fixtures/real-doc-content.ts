/**
 * Nội dung tài liệu THẬT cho seed docs (2026-05-27).
 *
 * Nội dung học thuật chính xác do tự soạn (công thức Toán/Lý/Hoá đúng, code
 * Python/JS chạy được, sample IELTS/TOEIC thực tế). KHÔNG phải placeholder.
 * Render multi-page PDF qua regenerate-real-pdfs.ts.
 *
 * Match theo keyword trong title (lowercase, không dấu). Doc không match dùng
 * fallback generic.
 */

export type Block =
  | { type: 'h'; text: string } // heading section
  | { type: 'p'; text: string } // paragraph
  | { type: 'b'; text: string } // bullet
  | { type: 'f'; text: string } // formula (mono, canh giữa box)
  | { type: 'code'; text: string }; // code block (mono)

export type DocContent = {
  /** Keyword (không dấu, lowercase) match trong title. */
  match: string;
  blocks: Block[];
};
...
export const REAL_CONTENTS: DocContent[] = [
  // ── Toán 12 — Đạo hàm ──────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Đạo hàm của hàm số y = f(x) tại điểm x₀ là giới hạn của tỉ số số gia hàm số trên số gia đối số khi số gia đối số dần về 0:' },
      { type: 'f', text: "f'(x₀) = lim(Δx→0) [f(x₀+Δx) − f(x₀)] / Δx" },
      { type: 'p', text: 'Về mặt hình học, f′(x₀) là hệ số góc của tiếp tuyến với đồ thị hàm số tại điểm (x₀; f(x₀)). Về mặt vật lý, đạo hàm biểu thị tốc độ biến thiên tức thời của đại lượng.' },
      ...
      { type: 'p', text: 'Hàm số đồng biến trên khoảng K khi f′(x) ≥ 0 với mọi x ∈ K (dấu = chỉ tại hữu hạn điểm). Nghịch biến khi f′(x) ≤ 0. Điểm cực trị là nghiệm của f′(x) = 0 mà f′ đổi dấu.' },
      { type: 'p', text: 'Ví dụ: y = x³ − 3x + 2. Ta có y′ = 3x² − 3 = 3(x−1)(x+1). y′ = 0 ⟺ x = ±1. Lập bảng biến thiên: hàm đạt cực đại tại x = −1 (y = 4), cực tiểu tại x = 1 (y = 0).' },
      { type: 'h', text: '5. Bài tập tự luyện' },
      { type: 'b', text: "Tính đạo hàm: y = (2x² + 1)⁵. ĐS: y' = 20x(2x²+1)⁴." },
      { type: 'b', text: 'Tìm cực trị của y = x⁴ − 2x². ĐS: cực tiểu tại x = ±1, cực đại tại x = 0.' },
  ...
  // ── Tích phân ──────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'F(x) là nguyên hàm của f(x) nếu F′(x) = f(x). Tích phân xác định trên [a;b] tính theo công thức Newton–Leibniz:' },
      ...
      { type: 'p', text: 'Đặt t = u(x) ⟹ dt = u′(x)dx. Đổi cận theo t. Ví dụ tính ∫₀¹ 2x·(x²+1)³ dx: đặt t = x²+1, dt = 2x dx. Khi x=0 thì t=1, x=1 thì t=2. Tích phân thành ∫₁² t³ dt = [t⁴/4]₁² = (16−1)/4 = 15/4.' },
      { type: 'h', text: '4. Phương pháp tính nhanh từng phần' },
      { type: 'f', text: '∫ u dv = u·v − ∫ v du' },
      { type: 'p', text: 'Quy tắc chọn u theo thứ tự ưu tiên LIATE (Logarit – Inverse – Đại số – Lượng giác – Mũ). Ví dụ ∫ x·eˣ dx: chọn u = x, dv = eˣdx ⟹ = x·eˣ − ∫ eˣ dx = eˣ(x−1) + C.' },
      { type: 'h', text: '5. Ứng dụng tính diện tích' },
      { type: 'p', text: 'Diện tích hình phẳng giới hạn bởi y = f(x), trục Ox, x = a, x = b: S = ∫ₐᵇ |f(x)| dx. Thể tích khối tròn xoay quanh Ox: V = π∫ₐᵇ f(x)² dx.' },
    ],
  },
  // ── Lượng giác lớp 11 ──────────────────────────────────────────
  {
  ...
  // ── Hình học Oxyz ──────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Mỗi điểm M trong không gian xác định bởi bộ ba toạ độ (x; y; z). Vectơ AB = (xB−xA; yB−yA; zB−zA). Độ dài: |AB| = √[(Δx)² + (Δy)² + (Δz)²].' },
      { type: 'h', text: '2. Tích vô hướng và tích có hướng' },
      { type: 'f', text: 'a·b = x₁x₂ + y₁y₂ + z₁z₂ = |a||b|cosφ' },
      { type: 'p', text: 'Tích có hướng [a,b] vuông góc với cả a và b, dùng để tính diện tích tam giác S = ½|[AB,AC]| và thể tích tứ diện V = ⅙|[AB,AC]·AD|.' },
      ...
      { type: 'p', text: 'Cho A(1;0;0), B(0;2;0), C(0;0;3). Viết phương trình mặt phẳng (ABC). Giải: mặt phẳng theo đoạn chắn x/1 + y/2 + z/3 = 1 ⟺ 6x + 3y + 2z − 6 = 0.' },
    ],
  },
  // ── Đề thi tốt nghiệp Toán ─────────────────────────────────────
  {
    match: 'tot nghiep thpt toan',
    blocks: [
      { type: 'h', text: 'ĐỀ THI TỐT NGHIỆP THPT — MÔN TOÁN' },
      { type: 'p', text: 'Thời gian làm bài: 90 phút, 50 câu trắc nghiệm. Dưới đây là một số câu tiêu biểu kèm lời giải.' },
      { type: 'h', text: 'Câu 1 (Đạo hàm)' },
      { type: 'p', text: 'Cho hàm y = x³ − 3x² + 1. Số điểm cực trị là? Giải: y′ = 3x² − 6x = 3x(x−2), y′=0 tại x=0 và x=2, đổi dấu 2 lần ⟹ 2 điểm cực trị. Đáp án: 2.' },
      { type: 'h', text: 'Câu 2 (Logarit)' },
      { type: 'p', text: 'Giải phương trình log₂(x−1) = 3. Giải: x − 1 = 2³ = 8 ⟹ x = 9. Đáp án: x = 9.' },
      { type: 'h', text: 'Câu 3 (Tích phân)' },
      { type: 'p', text: 'Tính I = ∫₀¹ (3x² + 2x) dx. Giải: I = [x³ + x²]₀¹ = 1 + 1 = 2. Đáp án: 2.' },
      { type: 'h', text: 'Câu 4 (Số phức)' },
      { type: 'p', text: 'Cho z = 3 − 4i. Tính |z|. Giải: |z| = √(3² + 4²) = √25 = 5. Đáp án: 5.' },
      { type: 'h', text: 'Câu 5 (Hình Oxyz)' },
      { type: 'p', text: 'Khoảng cách từ M(1;2;2) đến mặt phẳng (P): x + 2y + 2z − 3 = 0. Giải: d = |1+4+4−3|/√(1+4+4) = 6/3 = 2. Đáp án: 2.' },
    ],
  },
  // ── Vật lý — Dao động cơ ───────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Dao động điều hoà là dao động trong đó li độ là hàm cosin (hoặc sin) theo thời gian:' },
      ...
      { type: 'p', text: 'Vận tốc sớm pha π/2 so với li độ; gia tốc ngược pha với li độ. Hệ thức độc lập thời gian: A² = x² + v²/ω².' },
      { type: 'h', text: '3. Con lắc lò xo' },
      { type: 'f', text: 'ω = √(k/m),  T = 2π√(m/k)' },
      { type: 'p', text: 'Cơ năng W = ½kA² = ½mω²A² bảo toàn (bỏ qua ma sát). Động năng và thế năng biến thiên tuần hoàn với chu kỳ T/2.' },
      { type: 'h', text: '4. Con lắc đơn' },
      { type: 'f', text: 'T = 2π√(l/g)' },
      { type: 'p', text: 'Chu kỳ con lắc đơn chỉ phụ thuộc chiều dài l và gia tốc trọng trường g, không phụ thuộc khối lượng (dao động nhỏ, góc < 10°).' },
      { type: 'h', text: '5. Bài tập' },
      { type: 'p', text: 'Vật dao động x = 5cos(4πt) cm. Tìm v_max. Giải: A = 5cm = 0,05m; ω = 4π. v_max = Aω = 0,05·4π ≈ 0,628 m/s = 62,8 cm/s.' },
    ],
  },
  // ── Đề thi thử Vật lý ──────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Con lắc lò xo k = 100 N/m, m = 100g. Tính chu kỳ. Giải: T = 2π√(m/k) = 2π√(0,1/100) = 2π·0,0316 ≈ 0,2s.' },
      ...
      { type: 'p', text: 'Mạch RLC, công suất cực đại khi cộng hưởng: ZL = ZC ⟺ ω²LC = 1. Khi đó hệ số công suất cosφ = 1.' },
      { type: 'h', text: 'Câu 4 — Sóng ánh sáng' },
      { type: 'p', text: 'Khoảng vân i = λD/a. Với λ = 0,6μm, D = 2m, a = 1mm: i = 0,6·10⁻⁶·2/10⁻³ = 1,2·10⁻³ m = 1,2 mm.' },
    ],
  },
  // ── Hoá — Este Lipit ───────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Este là sản phẩm thay nhóm −OH của axit cacboxylic bằng nhóm −OR′. Công thức tổng quát este no đơn chức: CₙH₂ₙO₂ (n ≥ 2).' },
      ...
      { type: 'b', text: 'Thuỷ phân môi trường kiềm (xà phòng hoá, 1 chiều): RCOOR′ + NaOH → RCOONa + R′OH' },
      { type: 'h', text: '3. Lipit — Chất béo' },
      { type: 'p', text: 'Chất béo là trieste của glixerol C₃H₅(OH)₃ với axit béo (axit stearic C₁₇H₃₅COOH, axit oleic C₁₇H₃₃COOH...). Gọi là triglixerit.' },
      { type: 'p', text: 'Phản ứng xà phòng hoá chất béo tạo glixerol + muối natri của axit béo (xà phòng). Chỉ số xà phòng hoá đặc trưng cho chất béo.' },
      { type: 'h', text: '4. Bài tập' },
      { type: 'p', text: 'Xà phòng hoá hoàn toàn 8,8g etyl axetat (CH₃COOC₂H₅, M=88) cần bao nhiêu gam NaOH? Giải: n_este = 8,8/88 = 0,1 mol ⟹ n_NaOH = 0,1 mol ⟹ m = 0,1·40 = 4g.' },
    ],
  },
  // ── IELTS Speaking ─────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'You have 1 minute to prepare and 1–2 minutes to speak. Use the bullet points on the cue card to structure your answer, but always add personal details and examples.' },
  ...
  // ── IELTS Writing ──────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Task 2 requires a 250-word essay in 40 minutes. A clear 4-paragraph structure maximises your Coherence & Cohesion score.' },
  ...
  // ── TOEIC Reading ──────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Part 5 tests grammar and vocabulary. Read the sentence and choose the word that best completes it. Below are common question types with explanations.' },
      { type: 'h', text: 'Word form questions' },
      { type: 'p', text: 'Q: The new policy will have a significant ____ on productivity. (A) impact (B) impactful (C) impacted (D) impactfully. Answer: (A) impact — a noun is needed after the adjective "significant".' },
      { type: 'h', text: 'Verb tense questions' },
      { type: 'p', text: 'Q: By the time the manager arrived, the meeting ____ already. (A) starts (B) has started (C) had started (D) starting. Answer: (C) had started — past perfect for an action completed before another past action.' },
      { type: 'h', text: 'Preposition questions' },
      { type: 'p', text: 'Q: The report must be submitted ____ Friday. (A) in (B) on (C) at (D) by. Answer: (D) by — "by" indicates a deadline.' },
      { type: 'h', text: 'Tips' },
  ...
  // ── JLPT N5 Kanji ──────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'N5 yêu cầu khoảng 100 chữ Kanji cơ bản. Dưới đây là các chữ thường gặp kèm âm đọc và nghĩa.' },
      { type: 'h', text: 'Số đếm' },
  ...
  // ── Tiếng Anh giao tiếp ────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Tổng hợp mẫu câu thông dụng trong môi trường công sở theo từng tình huống.' },
  ...
  // ── Văn — Vợ chồng A Phủ ───────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Tô Hoài là cây bút văn xuôi tiêu biểu của văn học hiện đại Việt Nam. "Vợ chồng A Phủ" (1952) in trong tập "Truyện Tây Bắc", là kết quả chuyến đi thực tế cùng bộ đội giải phóng Tây Bắc.' },
      { type: 'h', text: '2. Nhân vật Mị' },
      { type: 'p', text: 'Mị là cô gái Mèo trẻ đẹp, tài hoa nhưng bị bắt làm con dâu gạt nợ nhà thống lí Pá Tra. Từ cô gái yêu đời, Mị trở nên câm lặng, "lùi lũi như con rùa nuôi trong xó cửa". Tuy nhiên sức sống tiềm tàng vẫn âm ỉ: đêm tình mùa xuân, tiếng sáo gọi bạn đánh thức khát vọng tự do; đêm cứu A Phủ là sự trỗi dậy mạnh mẽ của ý thức phản kháng.' },
      { type: 'h', text: '3. Nhân vật A Phủ' },
      { type: 'p', text: 'A Phủ là chàng trai mồ côi, gan góc, lao động giỏi. Vì đánh con quan mà bị phạt vạ, trở thành người ở trừ nợ. Khi để hổ ăn mất bò, A Phủ bị trói đứng chờ chết — chi tiết tố cáo sự tàn bạo của chế độ phong kiến miền núi.' },
      { type: 'h', text: '4. Giá trị tác phẩm' },
      { type: 'b', text: 'Giá trị hiện thực: phơi bày số phận bi thảm của người dân lao động dưới ách thống trị.' },
      { type: 'b', text: 'Giá trị nhân đạo: phát hiện và trân trọng sức sống, khát vọng tự do tiềm tàng.' },
      { type: 'b', text: 'Nghệ thuật: miêu tả tâm lí tinh tế, ngôn ngữ giàu chất thơ, đậm màu sắc Tây Bắc.' },
    ],
  },
  // ── Văn — Nghị luận xã hội ─────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'p', text: 'Bài NLXH thường có 2 dạng: nghị luận về tư tưởng đạo lí và nghị luận về hiện tượng đời sống. Dưới đây là dàn ý tổng quát.' },
      { type: 'h', text: '1. Mở bài' },
      { type: 'p', text: 'Dẫn dắt và nêu vấn đề cần nghị luận. Có thể dùng câu danh ngôn, câu hỏi gợi mở hoặc tình huống thực tế.' },
      { type: 'h', text: '2. Thân bài' },
      { type: 'b', text: 'Giải thích: làm rõ khái niệm, từ khoá của vấn đề.' },
      { type: 'b', text: 'Bàn luận: phân tích biểu hiện, nguyên nhân, ý nghĩa/tác hại; dẫn chứng thực tế.' },
      ...
      { type: 'p', text: 'Giải thích: biết ơn là sự ghi nhớ, trân trọng những gì người khác mang lại cho mình. Bàn luận: lòng biết ơn là nền tảng đạo đức, gắn kết con người (dẫn chứng "Uống nước nhớ nguồn", ngày 20/11, 27/7). Phản biện: phê phán thói vô ơn, sống ích kỷ. Bài học: thể hiện biết ơn bằng hành động cụ thể.' },
    ],
  },
];

/** Match content theo title (bỏ dấu, lowercase). null nếu không có. */
