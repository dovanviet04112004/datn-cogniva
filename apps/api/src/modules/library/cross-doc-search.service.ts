/**
 * CrossDocSearchService — port từ apps/web/src/lib/library/cross-doc-search.ts.
 * Search SEMANTIC across chunk của mọi doc (hybrid RRF: FTS + pgvector <=>),
 * granularity = chunk (page-level) khác hybridSearchLibraryDocs (doc-level).
 * SQL RRF copy NGUYÊN; embedQuery qua EmbeddingService (Voyage input_type
 * 'query' + retry 429 như embed-query.ts cũ).
 */
...
/**
 * Highlight query terms trong content excerpt với <mark>. HTML safe (escape
 * rồi mới wrap mark) — UI render dangerouslySetInnerHTML được vì
