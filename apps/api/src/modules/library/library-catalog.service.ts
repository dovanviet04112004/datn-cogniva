/**
 * LibraryCatalogService — port các route catalog UGC + karma:
 *   GET/POST /library/universities  (autocomplete + tạo dedup theo slug)
 *   GET/POST /library/courses       (autocomplete + tạo dedup theo (university, slug))
 *   GET      /library/karma/leaderboard
 * ILIKE giữ raw semantics (wildcard %/_ trong q đi thẳng — Prisma contains
 * escape sẽ lệch); full-row response map ĐÚNG thứ tự cột Drizzle schema.
 */
...
export class LibraryCatalogService {
  ...
  async listUniversities(q: string, limit: number) {
    const where = q.length > 0 ? Prisma.sql`WHERE (name ILIKE ${`%${q}%`} OR short_name ILIKE ${`%${q}%`})` : Prisma.empty;
    const rows = await this.prisma.$queryRaw<
      Array<{ id: string; slug: string; name: string; short_name: string | null; doc_count: number }>
    >(Prisma.sql`
  ...
  /** UGC tạo university — dedup theo slug ("ĐH Bách Khoa HN" gõ 2 lần → 1 row). */
  ...
  /** UGC tạo course — dedup theo (university | null, slug). */
  ...
  /* karmaLeaderboard KHÔNG port — route 0 caller (trang /library/karma SSR
     query DB trực tiếp qua lib getKarmaBoard). */
}

/** Full row library_university — ĐÚNG thứ tự cột Drizzle schema (route cũ trả `select()`/`.returning()`). */
...
/** Full row library_course — ĐÚNG thứ tự cột Drizzle schema. */
...
/**
 * Normalize tên VN → slug ascii để dedup — COPY từ
 * apps/web/src/lib/library/course-util.ts (slugifyVi). Đổi thì sửa cả 2.
 */
