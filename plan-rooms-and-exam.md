...
> **Triết lý:** Tự host hoàn toàn, không phụ thuộc SaaS bên ngoài. Production-grade từ ngày đầu. Stack: Next.js 15 + Drizzle + Mastra + LiveKit OSS self-hosted + Soketi + Yjs/Hocuspocus.
>
> **Vị trí:** Đây là V2 của Cogniva, nối tiếp `plan.md` (Phase 0-11). Sau khi Launch (Phase 11) xong, bắt đầu Phase 12. Tổng effort: 8 tuần full-time hoặc 16 tuần part-time.
...
- Real-time chat (WebSocket via Soketi self-hosted).
...
└────────────────────┬───────────────────────────┘
       ...
       ▼             ▼             ▼              ▼
┌────────────┐ ┌────────────┐ ┌──────────┐ ┌────────────┐
│  LiveKit   │ │  Soketi    │ │Hocuspocus│ │  Next.js   │
...
| Soketi      | I/O-bound, nhiều conn| Horizontal, sticky sessions    |
...
- Deploy 4 service self-hosted bằng Docker Compose: LiveKit, coturn, Soketi, Hocuspocus.
- Domain DNS + SSL (Let's Encrypt) cho `livekit.`, `turn.`, `soketi.`, `hocus.` subdomain.
- Reverse proxy (Caddy) trước Soketi/Hocuspocus (LiveKit + coturn dùng host network nên không qua proxy).
...
**`infrastructure/soketi/docker-compose.yml`**
```yaml
# Pusher-compatible WS service cho chat/realtime app events
services:
  soketi:
    image: quay.io/soketi/soketi:1-16-debian
    ports:
      - "6001:6001"
      - "9601:9601"   # metrics
    environment:
      DEFAULT_APP_ID: cogniva
...
```
# Soketi và Hocuspocus có WS gateway cần TLS termination
soketi.cogniva.com {
...
# Soketi
NEXT_PUBLIC_SOKETI_HOST=soketi.cogniva.com
NEXT_PUBLIC_SOKETI_KEY=app-key-xxx
SOKETI_APP_ID=cogniva
SOKETI_SECRET=app-secret-xxx
...
- [ ] DNS resolve `livekit.cogniva.com`, `turn.cogniva.com`, `soketi.cogniva.com`, `hocus.cogniva.com`.
- [ ] Let's Encrypt cert valid (kiểm tra qua `curl -v https://soketi.cogniva.com`).
...
- [ ] Soketi metrics endpoint `:9601/metrics` trả Prometheus format.
...
- Firewall (UFW) chỉ mở: 80/443 (app), 7880/7881 (LK signaling), 50000-60000/udp (LK media), 3478/5349 (TURN), 6001 (Soketi qua Caddy → đóng port direct).
...
if [[ "$ROLE" == "app" ]]; then
  # Soketi + Hocuspocus đi qua Caddy 443 → không expose direct port
...
| A    | soketi            | <APP_IP>      | ✓     | Auto | WS qua Caddy → Cloudflare OK   |
| A    | hocus             | <APP_IP>      | ✓     | Auto | WS qua Caddy → Cloudflare OK   |
| AAAA | (same)            | <IPv6>        | match | Auto | Dual-stack                     |

**Quan trọng:** LiveKit + coturn KHÔNG được proxy qua Cloudflare (Cloudflare không hỗ trợ WebRTC UDP relay). Soketi/Hocuspocus có thể proxy vì là pure WS over 443.
...
curl -fsS https://soketi.cogniva.com/usage >/dev/null || fail 'Soketi down'
  ...
  switch (event.event) {
    ...
    case 'room_finished':
      await db.update(rooms).set({ endedAt: new Date(), status: 'ENDED' })
        .where(eq(rooms.id, event.room!.name));
      // Phase 15 sẽ trigger AI summary qua Inngest ở đây
      break;
...
4. Client redirect → /rooms/[id]/waiting (poll status mỗi 3s hoặc subscribe Soketi)
5. Mod thấy badge "1 người chờ" → click Approve → status = 'ACTIVE'
6. Soketi broadcast `room:approved` đến PENDING user → client tự fetch token + redirect
```
...
- Chat realtime qua Soketi: gửi/nhận text, file (link upload R2), system events.
...
```typescript
/**
 * Pomodoro đồng bộ — state share qua LiveKit data channel (lightweight, không cần Soketi).
...
- [ ] Soketi crash → chat hiển thị "Mất kết nối", auto retry sau 5s.
...
- Soketi presence channel sign bằng app secret server-side, client không có.
...
- AI response stream từng token qua Soketi event `ai:streaming`.
- LiveKit Egress trigger qua API → record composite (video + audio mixed) layout `speaker`.
- Inngest workflow xử lý recording: extract audio → Whisper transcribe → AI summary → auto-chapter → flashcard gen.
...
### 15.4. Post-processing pipeline (Inngest)
...
- [ ] Inngest dashboard: function `process-recording` chạy success, transcript + summary trong DB.
...
- Real-time leaderboard qua Redis sorted set + Soketi broadcast.
...
export default function () {
  ...
  // subscribe Soketi WS + answer questions
  sleep(30);
}
```
...
- [ ] Prometheus scrape Soketi `:9601`, LiveKit Prom export.
- [ ] Grafana dashboard: P95 latency, error rate, concurrent rooms.
- [ ] Sentry capture từ frontend + Inngest functions.
...
Hetzner CCX23 (4 vCPU, 16GB) × 4 servers       = $120/mo
  ├ 2 cho LiveKit
  ├ 1 cho coturn + Soketi
  └ 1 cho Hocuspocus + Inngest workers
...
*Plan v2.0 — Phase 12-19 nối tiếp Cogniva plan.md (Phase 0-11). Update khi build thực tế.*
